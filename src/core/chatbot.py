...
from src.config.prompts import get_employer_qa_prompt
...
class EmployerQAChatbot:
    ...
    def answer_question(self, question: str, history: List[Dict[str, Any]]) -> str:
        ...
        try:
            ...
            # Format the prompt
            prompt = get_employer_qa_prompt(context, question)
            
            # Prepare messages for the LLM
            messages = []
            
            # Add system context from the prompt (first part)
            system_content = prompt.split("**Resume Context:**")[0].strip()
